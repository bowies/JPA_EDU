...
(function () {
    ...
    $.fn.w2grid = function(method) {
        if (typeof method === 'object' || !method ) {
            ...
            for (var r in records) {
                if (records[r][method.recid] == null || typeof records[r][method.recid] == 'undefined') {
    ...
    w2grid.prototype = {
        ...
        add: function (record) {
            ...
            var added = 0;
            for (var o in record) {
                if (record[o][this.recid] == null || typeof record[o][this.recid] == 'undefined') {
        ...
        find: function (obj, returnIndex) {
            ...
            for (var i = 0; i < this.records.length; i++) {
                ...
                if (match && returnIndex !== true) recs.push(this.records[i][this.recid]);
        ...
        set: function (recid, record, noRefresh) { // does not delete existing, but overrides on top of it
            ...
            if (recid == null) {
                ...
                var isSummary = (this.records[ind] && this.records[ind][this.recid] == recid ? false : true);
                if (isSummary) {
        ...
        get: function (recid, returnIndex) {
            // search records
            for (var i = 0; i < this.records.length; i++) {
                if (this.records[i][this.recid] == recid) {
                    if (returnIndex === true) return i; else return this.records[i];
                }
            }
            // search summary
            for (var i = 0; i < this.summary.length; i++) {
                if (this.summary[i][this.recid] == recid) {
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                for (var r = this.records.length-1; r >= 0; r--) {
                    if (this.records[r][this.recid] == arguments[a]) { this.records.splice(r, 1); removed++; }
                }
            }
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            if (!url) {
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                var record = this.get(recid);
                if (record == null) continue;
                var index  = this.get(record[this.recid], true);
                var recEl  = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid));
                if (this.selectType == 'row') {
        ...
        selectNone: function () {
            ...
            for (var s in sel.indexes) {
                ...
                var recid = rec ? rec[this.recid] : null;
                var recEl = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(recid));
                recEl.removeClass('w2ui-selected').removeData('selected');
                recEl.find('.w2ui-grid-select-check').prop("checked", false);
                // for not rows
        ...
        getSelection: function (returnIndex) {
            ...
            if (this.selectType == 'row') {
                for (var s in sel.indexes) {
                    ...
                    if (returnIndex === true) ret.push(sel.indexes[s]); else ret.push(this.records[sel.indexes[s]][this.recid]);
                }
                return ret;
            } else {
                for (var s in sel.indexes) {
                    ...
                    if (!this.records[sel.indexes[s]]) continue;
                    for (var c in cols) {
                        ret.push({ recid: this.records[sel.indexes[s]][this.recid], index: parseInt(sel.indexes[s]), column: cols[c] });
                    }
                }
                return ret;
            }
        ...
        requestComplete: function(status, cmd, callBack) {
            ...
            if (status != 'error') {
                ...
                if (typeof responseText != 'undefined' && responseText != '') {
                    ...
                    // convert page
                    else if ((typeof data === 'object')&&(data.content !== null)&&(data.size !== null)) {
                    	data = {
        ...
        getChanges: function () {
            ...
            for (var r in this.records) {
                ...
                if (typeof rec['changes'] != 'undefined') {
                    changes.push($.extend(true, { recid: rec[this.recid] }, rec.changes));
                }
            }
            return changes;
        },
        ...
        mergeChanges: function () {
            ...
            for (var c in changes) {
                var record = this.get(changes[c][this.recid]);
                for (var s in changes[c]) {
                    if (s == this.recid) continue; // do not allow to change recid
                            ...
                            case 9:  // tab
                                ...
                                if (next_col == null) {
                                    ...
                                    if (tmp != null && tmp != index) {
                                        next_rec = obj.records[tmp][obj.recid];
                            ...
                            case 13: // enter
                                ...
                                if (next != null && next != index) {
                                    setTimeout(function () {
                                        if (obj.selectType != 'row') {
                                            obj.selectNone();
                                            obj.select({ recid: obj.records[next][obj.recid], column: column });
                                        } else {
                                            obj.editField(obj.records[next][obj.recid], column, null, event);
                                        }
                                    }, 100);
                                }
                                break;
        ...
        editChange: function (el, index, column, event) {
            ...
            var tr      = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(rec[this.recid]));
            ...
            var eventData = {
                phase: 'before', type: 'change', target: this.name, input_id: el.id, recid: rec[this.recid], index: index, column: column,
            ...
            if (url) {
                ...
                if (typeof recs[0] != 'object') {
                    ...
                    for (var r in recs) {
                        ...
                        var ind = this.get(recs[r][this.recid], true);
                        if (ind != null && fld != this.recid) {
        ...
        click: function (recid, event) {
            ...
            if (event.shiftKey && sel.length > 0 && obj.multiSelect) {
                ...
                for (var i = start; i <= end; i++) {
                    ...
                    if (this.selectType == 'row') {
                        sel_add.push(this.records[i][this.recid]);
                    } else {
                        for (var sc in selectColumns) sel_add.push({ recid: this.records[i][this.recid], column: selectColumns[sc] });
                    }
        ...
        keydown: function (event) {
            ...
            var recEL  = $('#grid_'+ obj.name +'_rec_'+ (ind !== null ? w2utils.escapeId(obj.records[ind][obj.recid]) : 'none'));
            var cancel = false;
            var key    = event.keyCode;
            ...
            switch (key) {
                ...
                case 37: // left
                    ...
                    var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind][obj.recid])).parents('tr');
                    if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                ...
                case 38: // up
                    ...
                    if (prev != null) {
                        ...
                        if (obj.records[prev].expanded) {
                            var subgrid = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[prev][obj.recid]) +'_expanded_row').find('.w2ui-grid');
                            if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                                obj.selectNone();
                                var grid = subgrid.attr('name');
                                var recs = w2ui[grid].records;
                                w2utils.keyboard.active(grid);
                                w2ui[grid].click(recs[recs.length-1][obj.recid]);
                                cancel = true;
                                break;
                            }
                        }
                        if (shiftKey && obj.multiSelect) { // expand selection
                            ...
                            if (obj.selectType == 'row') {
                                if (obj.last.sel_ind > prev && obj.last.sel_ind != ind2) {
                                    obj.unselect(obj.records[ind2][obj.recid]);
                                } else {
                                    obj.select(obj.records[prev][obj.recid]);
                                }
                            } else {
                                if (obj.last.sel_ind > prev && obj.last.sel_ind != ind2) {
                                    prev = ind2;
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ recid: obj.records[prev][obj.recid], column: columns[c] });
                            ...
                            if (sel.length > 300) this.selectNone(); else this.unselect.apply(this, sel);
                            obj.click({ recid: obj.records[prev][obj.recid], column: columns[0] }, event);
                        }
                        ...
                        var parent = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[ind][obj.recid])).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var recid = parent.prev().attr('recid');
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                ...
                case 40: // down
                    ...
                    if (obj.records[ind2].expanded) {
                        var subgrid = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2][obj.recid]) +'_expanded_row').find('.w2ui-grid');
                        if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                            obj.selectNone();
                            var grid = subgrid.attr('name');
                            var recs = w2ui[grid].records;
                            w2utils.keyboard.active(grid);
                            w2ui[grid].click(recs[0][obj.recid]);
                            cancel = true;
                            break;
                        }
                    }
                    ...
                    if (next != null) {
                        if (shiftKey && obj.multiSelect) { // expand selection
                            ...
                            if (obj.selectType == 'row') {
                                if (this.last.sel_ind < next && this.last.sel_ind != ind) {
                                    obj.unselect(obj.records[ind][obj.recid]);
                                } else {
                                    obj.select(obj.records[next][obj.recid]);
                                }
                            } else {
                                if (this.last.sel_ind < next && this.last.sel_ind != ind) {
                                    next = ind;
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ recid: obj.records[next][obj.recid], column: columns[c] });
                                    obj.unselect.apply(obj, tmp);
                                } else {
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ recid: obj.records[next][obj.recid], column: columns[c] });
                            ...
                            if (sel.length > 300) this.selectNone(); else this.unselect.apply(this, sel);
                            obj.click({ recid: obj.records[next][obj.recid], column: columns[0] }, event);
                        }
                        ...
                        var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2][obj.recid])).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var recid = parent.next().attr(obj.recid);
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                            w2utils.keyboard.active(grid);
                            w2ui[grid].click(recid);
            ...
            function selectTopRecord() {
                ...
                obj.select({ recid: obj.records[ind][obj.recid], column: 0});
            }
            ...
            function tmpUnselect () {
                ...
                if (obj.selectType != 'row') {
                    ...
                    obj.last.sel_type = 'key';
                    if (sel.length > 1) {
                        sel.splice(sel.indexOf(obj.records[obj.last.sel_ind][obj.recid]), 1);
                        obj.unselect.apply(obj, sel);
                        return true;
                    }
                    return false;
        ...
        paste: function (text) {
            ...
            for (var t in text) {
                ...
                for (var c in cols) newSel.push({ recid: rec[this.recid], column: cols[c] });
                ind++;
            }
            this.selectNone();
        ...
        refreshCell: function (recid, field) {
            var index     = this.get(recid, true);
            var isSummary = (this.records[index] && this.records[index][this.recid] == recid ? false : true);
                ...
                for (var i = ind1; i <= ind2; i++) {
                    ...
                    if (obj.selectType != 'row') {
                        ...
                        for (var c = col1; c <= col2; c++) {
                            ...
                            newSel.push({ recid: obj.records[i][obj.recid], column: parseInt(c) });
                        }
                    } else {
                        newSel.push(obj.records[i][obj.recid]);
                    }
                }
                if (obj.selectType != 'row') {
                    var sel = obj.getSelection();
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            if (!record) return '';
            var id = w2utils.escapeId(record[this.recid]);
            var isRowSelected = false;
            if (sel.indexes.indexOf(ind) != -1) isRowSelected = true;
            // render TR
            rec_html += '<tr id="grid_'+ this.name +'_rec_'+ record[this.recid] +'" recid="'+ record[this.recid] +'" line="'+ lineNum +'" '+
                ...
                (summary !== true ?
                    (w2utils.isIOS ?
                        '    onclick  = "w2ui[\''+ this.name +'\'].dblClick(\''+ record[this.recid] +'\', event);"'
                        :
                        '    onclick  = "w2ui[\''+ this.name +'\'].click(\''+ record[this.recid] +'\', event);"'+
                        '    oncontextmenu = "w2ui[\''+ this.name +'\'].contextMenu(\''+ record[this.recid] +'\', event);"'
                     )
                    : ''
                ) +
            ...
            if (this.show.selectColumn) {
                rec_html +=
                        ...
                        '        onclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            ...
                            '                if (this.checked) obj.select(\''+ record[this.recid] + '\'); else obj.unselect(\''+ record[this.recid] + '\'); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '    </div>'
                            :
                            '' ) +
            ...
            if (this.show.expandColumn) {
                ...
                rec_html +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_expand' + (summary ? '_s' : '') + '" class="w2ui-grid-data w2ui-col-expand">'+
                            ...
                            '            onclick="w2ui[\''+ this.name +'\'].toggle(\''+ record[this.recid] +'\', event); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '        '+ tmp_img +' </div>'
                            :
                            '' ) +
        ...
        status: function (msg) {
            if (typeof msg != 'undefined') {
                ...
                if (sel.length > 0) {
                    ...
                    var tmp = sel[0];
                    if (typeof tmp == 'object') tmp = tmp[this.recid] + ', '+ w2utils.lang('Column') +': '+ tmp.column;
